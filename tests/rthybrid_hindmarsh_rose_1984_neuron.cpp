...
TEST(RTHYBRID_HINDMARSH_ROSE_1984_NEURON, DescriptorShape)
{
    const auto *descriptor = rtsyn_rthybrid_hindmarsh_rose_1984_neuron_get_descriptor();
    ASSERT_NE(descriptor, nullptr);
    EXPECT_STREQ(descriptor->name, "rthybrid_hindmarsh_rose_1984_neuron");
    EXPECT_EQ(descriptor->node_type, RTSYN_ABI_NODE_PLUGIN);
    EXPECT_EQ(descriptor->port_count, 4U);
    EXPECT_EQ(descriptor->param_count, 12U);
    EXPECT_EQ(descriptor->state_count, 3U);
}

TEST(RTHYBRID_HINDMARSH_ROSE_1984_NEURON, ProcessesAndPublishesFiniteOutput)
{
    const auto *descriptor = rtsyn_rthybrid_hindmarsh_rose_1984_neuron_get_descriptor();
...
TEST(RTHYBRID_HINDMARSH_ROSE_1984_NEURON, StateAndInvalidAccess)
{
    const auto *descriptor = rtsyn_rthybrid_hindmarsh_rose_1984_neuron_get_descriptor();
    void *instance = nullptr;
    double value = 1.0;
